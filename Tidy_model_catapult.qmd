...
```{r}
#| label: phase3-factor-importance
#| tbl-cap: "Factor Importance Comparison: Full vs Fractional Factorial"

# Type II ANOVA for FULL factorial
type2_full <- Anova(model_1, type = "II")

type2_full_results <- type2_full %>%
  as.data.frame() %>%
  rownames_to_column("Factor") %>%
  filter(Factor != "Residuals") %>%
  mutate(
    Factor = case_when(
      Factor == "LaunchAngle_deg" ~ "Launch Angle",
      Factor == "ArmLength_cm" ~ "Arm Length",
      Factor == "RubberBands_count" ~ "Rubber Bands"
    ),
    Variance_Percent = (`Sum Sq` / sum(`Sum Sq`)) * 100
  ) %>%
  arrange(desc(`F value`))

# Type II ANOVA for FRACTIONAL
type2_frac <- Anova(model_fractional, type = "II")

type2_frac_results <- type2_frac %>%
  as.data.frame() %>%
  rownames_to_column("Factor") %>%
  filter(Factor != "Residuals") %>%
  mutate(
...
type2_full_results %>%
  select(Factor, Full_F = `F value`, Full_Var = Variance_Percent, Full_p = `Pr(>F)`) %>%
  left_join(
    type2_frac_results %>% 
      select(Factor, Frac_F = `F value`, Frac_Var = Variance_Percent, Frac_p = `Pr(>F)`),
    by = "Factor"
  ) %>%
  arrange(desc(Full_F)) %>%
  mutate(
    across(c(Full_F, Frac_F), ~round(., 1)),
    across(c(Full_Var, Frac_Var), ~round(., 1)),
    across(c(Full_p, Frac_p), ~ifelse(. < 0.001, "<0.001", round(., 3)))
  ) %>%
  kable(col.names = c("Factor", "F-value", "Var%", "p-value", 
                      "F-value", "Var%", "p-value")) %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE) %>%
  add_header_above(c(" " = 1, "Full Factorial (24 runs)" = 3, "Fractional (12 runs)" = 3))
```
...
plot_data_full <- type2_full_results %>%
  mutate(Design = "Full Factorial\n(24 runs)")

plot_data_frac <- type2_frac_results %>%
  mutate(Design = "Fractional Factorial\n(12 runs)")

plot_data <- bind_rows(plot_data_full, plot_data_frac) %>%
  mutate(Factor = fct_reorder(Factor, Variance_Percent, .desc = FALSE))

# Create side-by-side comparison plot
ggplot(plot_data, aes(x = Variance_Percent, y = Factor, fill = Design)) +
  ...
  scale_fill_manual(values = c("Full Factorial\n(24 runs)" = "#2E86AB",
                                "Fractional Factorial\n(12 runs)" = "#F18F01")) +
  labs(
    title = "Factor Importance Comparison: Full vs Fractional Factorial",
  ...
  xlim(0, max(plot_data$Variance_Percent) * 1.15)
```
...
```{r}
#| label: phase2-recommendation

# Find best model by AIC (from full factorial models)
best_model_idx <- model_comparison %>% 
  filter(Design == "Full (24 runs)") %>% 
  slice_min(AIC, n = 1) %>%
  pull(Model)

best_stats <- model_comparison %>% filter(Model == best_model_idx)

cat("RECOMMENDED MODEL:", best_model_idx, "\n\n")

cat("Model Performance:\n")
cat("  - R² =", round(best_stats$R_squared * 100, 1), "%\n")
cat("  - Adjusted R² =", round(best_stats$Adj_R_squared * 100, 1), "%\n")
cat("  - RMSE =", round(best_stats$RMSE, 2), "cm\n")
cat("  - AIC =", round(best_stats$AIC, 1), "\n\n")

cat("FACTOR IMPORTANCE RANKING:\n")
for(i in 1:nrow(type2_full_results)) {
  cat(sprintf("  %d. %s (F=%.1f, p%s, %.1f%% variance)\n", 
              i, 
              type2_full_results$Factor[i],
              type2_full_results$`F value`[i],
              ifelse(type2_full_results$`Pr(>F)`[i] < 0.001, "<0.001", 
                     paste0("=", round(type2_full_results$`Pr(>F)`[i], 3))),
              type2_full_results$Variance_Percent[i]))
}
```
